...
class MockSearchIOError_Base(Exception):
    pass
...
class TestBlastToTable(unittest.TestCase):
    ...
    def setUp(self):
        if (
            not hasattr(sys.modules["Bio.SearchIO"], "SearchIOError")
            or sys.modules["Bio.SearchIO"].SearchIOError != MockSearchIOError_Base
        ):
            sys.modules["Bio.SearchIO"].SearchIOError = MockSearchIOError_Base

    def _create_mock_hsp(
        self,
        q_start,
        q_end,
        h_start,
    ...
    def _run_main_with_mocked_searchio(
        self, mock_searchio_parse_method_instance, input_file_path="dummy_blast.xml"
    ):
        ...
        with patch.object(sys, "argv", ["blast_to_table.py", input_file_path]):
            ...
            return (
                ...
                mock_searchio_parse_method_instance,
            )
    ...
    def test_single_query_single_hit_single_hsp(self):
        mock_hsp1 = self._create_mock_hsp(
            q_start=0,
            q_end=100,
            h_start=20,
            h_end=120,
        ...
        mock_hit1 = self._create_mock_hit(
            hit_id="subject1",
            description="Subject One description",
            seq_len=200,
            hsps=[mock_hsp1],
        )
        mock_query1 = self._create_mock_query_result(
            query_id="queryA", hits=[mock_hit1]
        )
    ...
    def test_exclude_self_hit(self):
        mock_hsp_self = self._create_mock_hsp(
            0, 50, 0, 50, 1e-5, 50, 50, 50, query_id="queryA", hit_id="queryA"
        )
        mock_hit_self = self._create_mock_hit(
            "queryA", "Self hit description", 100, [mock_hsp_self]
        )
    ...
    def test_multiple_hsps_for_single_hit(self):
        ...
        mock_hsp2 = self._create_mock_hsp(
            q_start=60,
            q_end=100,
            h_start=70,
            h_end=110,
        ...
        mock_hit1 = self._create_mock_hit(
            "subjectB", "Subject B", 150, [mock_hsp1, mock_hsp2]
        )
        ...
        stdout, stderr, _ = self._run_main_with_mocked_searchio(mock_parse_method)
        row1_expected = (
            "queryA\tSubject\tSubject\t150\tB\t1.00\t-5\t90.00\t96.00\t1\t50\t11\t60"
        )
    ...
    def test_multiple_hits_for_single_query(self):
        ...
        stdout, stderr, _ = self._run_main_with_mocked_searchio(mock_parse_method)
        row1_expected = (
            "queryA\tHit\tHit\t100\tX\t1.00\t-3\t100.00\t100.00\t1\t30\t1\t30"
        )
        row2_expected = (
            "queryA\tHit\tHit\t110\tY\t1.00\t-4\t100.00\t100.00\t1\t40\t1\t40"
        )
        self.assertIn(self.header_string, stdout)
        self.assertIn(row1_expected, stdout)
        self.assertIn(row2_expected, stdout)
    ...
    def test_multiple_query_results(self):
        ...
        mock_hit_q1h1 = self._create_mock_hit(
            "q1h1", "Query1 Hit1", 80, [mock_hsp_q1h1]
        )
        ...
        mock_hit_q2h1 = self._create_mock_hit(
            "q2h1", "Query2 Hit1", 90, [mock_hsp_q2h1]
        )
        ...
        stdout, stderr, _ = self._run_main_with_mocked_searchio(mock_parse_method)
        row1_expected = (
            "query1\tQuery1\tQuery1\t80\tHit1\t1.00\t-2\t100.00\t100.00\t1\t20\t1\t20"
        )
        row2_expected = (
            "query2\tQuery2\tQuery2\t90\tHit1\t1.00\t-3\t100.00\t100.00\t1\t25\t1\t25"
        )
        self.assertIn(self.header_string, stdout)
        self.assertIn(row1_expected, stdout)
        self.assertIn(row2_expected, stdout)
    ...
    def test_evalue_variations(self):
        ...
        mock_hit = self._create_mock_hit(
            "hit_eval", "Evalue Test", 50, [mock_hsp_zero_eval, mock_hsp_large_exp]
        )
    ...
    def test_header_parsing_variations(self):
        headers_to_test = {
            "gi|123|gb|U00001.1|LOCUS_NAME Description": (
                "U00001",
                "LOCUS_NAME",
                "Description",
            ),
            "sp|P12345|PROT_NAME Protein description": (
                "P12345",
                "PROT_NAME",
                "Protein description",
            ),
        ...
        for header_str, (
            ...
            expected_locus,
            expected_desc,
        ) in headers_to_test.items():
            hit_id = f"hit_header_{idx}"
            mock_hsp = self._create_mock_hsp(
                0,
                10,
                0,
                10,
        ...
        mock_query = self._create_mock_query_result(
            "query_header_test", all_hits_for_query
        )
        mock_parse_method = MagicMock(return_value=[mock_query])
        stdout, _, _ = self._run_main_with_mocked_searchio(mock_parse_method)
    ...
    def test_file_not_found(self):
        ...
        stdout, stderr, mock_parse_call = self._run_main_with_mocked_searchio(
            mock_parse_method, input_file_path="nonexistent.xml"
        )
    ...
    def test_invalid_blast_file_content(self):
        mock_parse_method = MagicMock(
            side_effect=MockSearchIOError_Base("Invalid BLAST content!")
        )
        stdout, stderr, mock_parse_call = self._run_main_with_mocked_searchio(
            mock_parse_method, input_file_path="invalid.xml"
        )

        self.assertIn(
            "Error parsing BLAST file invalid.xml: Invalid BLAST content!", stderr
        )
        self.assertIn(
            "Please ensure the file is a valid BLAST text output format.", stderr
        )
        self.assertIn(self.header_string, stdout)
