...
class TestFindTandemRepeats(unittest.TestCase):
    ...
    def test_arg_parser_pattern_required(self):
        with patch.object(
            sys, "argv", ["find_tandem_repeats.py", "--sequence", "ACGT"]
        ):
    ...
    def test_arg_parser_sequence_source_required(self):
        with patch.object(sys, "argv", ["find_tandem_repeats.py", "--pattern", "AA"]):
            ...
            self.assertIn(
                "Must provide either --sequence or --sequence_file", r_err.getvalue()
            )
    ...
    def test_arg_parser_fudge_default(self):
        with patch.object(
            sys,
            "argv",
            ["find_tandem_repeats.py", "--pattern", "AA", "--sequence", "AAAA"],
        ), patch(
            "pylib.scripts.find_tandem_repeats.process_sequence", return_value=None
        ) as mock_process:
            find_tandem_repeats.main()
    ...
    def test_input_from_sequence_argument(self, mock_process_sequence):
        ...
        with patch.object(
            sys,
            "argv",
            ["find_tandem_repeats.py", "--pattern", "GA", "--sequence", test_seq],
        ):
            find_tandem_repeats.main()
    ...
    def test_input_from_fasta_file(self, mock_process_sequence, mock_parse_fasta):
        ...
        mock_parse_fasta.return_value = [
            MockSeqRecord(fasta_content_seq, id=fasta_content_id)
        ]
        with patch.object(
            sys,
            "argv",
    ...
    @patch(
        ...
        side_effect=FileNotFoundError("File not found mock"),
    )
    @patch("sys.exit")
    def test_input_sequence_file_not_found(self, mock_sys_exit, mock_parse_fasta):
        ...
        with patch.object(
            sys,
            "argv",
            ["find_tandem_repeats.py", "--pattern", "AA", "--sequence_file", file_path],
        ), redirect_stderr(io.StringIO()) as stderr_capture:
            find_tandem_repeats.main()
        self.assertIn(
            f"Error: Sequence file '{file_path}' not found.", stderr_capture.getvalue()
        )
    ...
    def test_pattern_empty_error(self):
        with patch.object(
            sys,
            "argv",
            ["find_tandem_repeats.py", "--pattern", "", "--sequence", "ACGT"],
        ), self.assertRaises(SystemExit) as cm, redirect_stderr(
            io.StringIO()
        ) as stderr_capture:
            find_tandem_repeats.main()
        self.assertEqual(cm.exception.code, 1)
    ...
    def test_pattern_and_sequence_case_insensitivity(self, mock_process_sequence):
        with patch.object(
            sys,
            "argv",
            ["find_tandem_repeats.py", "--pattern", "at", "--sequence", "aTaTcGat"],
        ):
            find_tandem_repeats.main()
    ...
    def _run_and_capture_output(self, args_list):
        with patch.object(sys, "argv", args_list), redirect_stdout(
            io.StringIO()
        ) as stdout_capture, redirect_stderr(
            io.StringIO()
        ):  # Suppress verbose if not testing for it specifically
            try:
                find_tandem_repeats.main()
            except (
                SystemExit
            ):  # Catch sys.exit if it occurs, but we primarily check stdout
                pass
            return stdout_capture.getvalue()

    def test_no_repeats_found(self):
        output = self._run_and_capture_output(
            ["find_tandem_repeats.py", "--pattern", "XXX", "--sequence", "ACGTACGT"]
        )
    ...
    def test_simple_exact_repeats_aaa(self):
        ...
        output = self._run_and_capture_output(
            ["find_tandem_repeats.py", "--pattern", "AAA", "--sequence", "CGAAACG"]
        )
    ...
    def test_simple_exact_repeats_at_at_at(self):
        ...
        output = self._run_and_capture_output(
            ["find_tandem_repeats.py", "--pattern", "AT", "--sequence", "CGATATATATGCG"]
        )
    ...
    def test_fudge_factor_joins_blocks(self):
        ...
        self.assertIn(
            "Found 2 instances of pattern XX", output
        )  # Instances are still distinct matches
        self.assertIn("Found 1 blocks of repeats", output)
    ...
    def test_fudge_factor_ta_various(self):
        ...
        output1 = self._run_and_capture_output(
            [
                "find_tandem_repeats.py",
                "--pattern",
                "TA",
        ...
        output2 = self._run_and_capture_output(
            [
                "find_tandem_repeats.py",
                "--pattern",
                "TA",
        ...
        output3 = self._run_and_capture_output(
            [
                "find_tandem_repeats.py",
                "--pattern",
                "TA",
    ...
    def test_multiple_distinct_blocks(self):
        ...
        output_at = self._run_and_capture_output(
            [
                "find_tandem_repeats.py",
                "--pattern",
                "AT",
        ...
        output_ag = self._run_and_capture_output(
            [
                "find_tandem_repeats.py",
                "--pattern",
                "AG",
    ...
    def test_edge_cases_pattern_location(self):
        # Pattern at start
        output_start = self._run_and_capture_output(
            [
                "find_tandem_repeats.py",
                "--pattern",
                "XXX",
        ...
        # Pattern at end
        output_end = self._run_and_capture_output(
            [
                "find_tandem_repeats.py",
                "--pattern",
                "XXX",
        ...
        output_equal = self._run_and_capture_output(
            [
                "find_tandem_repeats.py",
                "--pattern",
                "ACGT",
    ...
    def test_verbose_output(self):
        ...
        with patch.object(sys, "argv", args), redirect_stdout(
            io.StringIO()
        ) as stdout_capture:
            find_tandem_repeats.main()
        ...
        self.assertIn(
            "VERBOSE: Continuing current block. Repeats in block: 2", output
        )  # For match at offset 3
        self.assertIn(
            "VERBOSE: Continuing current block. Repeats in block: 3", output
        )  # For match at offset 5
        self.assertIn(
            "VERBOSE: Ending current block. Repeats: 3, Start: 0, End: 6", output
        )
