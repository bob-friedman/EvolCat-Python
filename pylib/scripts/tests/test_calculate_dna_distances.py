...
class TestCalculateDnaDistances(unittest.TestCase):
    ...
    @patch("sys.exit")
    def test_arg_parser_accepts_input_file_and_handles_not_found(
        self, mock_sys_exit, mock_parse_fasta
    ):
        dummy_fasta_path = "non_existent.fasta"
        with patch.object(
            sys, "argv", ["calculate_dna_distances.py", dummy_fasta_path]
        ):
        ...
        self.assertIn(
            f"Error: File not found: {dummy_fasta_path}", stderr_capture.getvalue()
        )
    ...
    def test_sequences_different_lengths_error(self, mock_sys_exit, mock_parse_fasta):
        ...
        mock_sys_exit.assert_called_once_with(1)
        self.assertIn(
            "Error: Sequences must be of the same length for distance calculation.",
    ...
    def test_fewer_than_two_sequences_error(self, mock_sys_exit, mock_parse_fasta):
        mock_parse_fasta.return_value = [MockSeqRecord("ACGT", id="seq1")]
        with patch.object(sys, "argv", ["calculate_dna_distances.py", "dummy.fasta"]):
            with redirect_stderr(io.StringIO()) as stderr_capture:
                calculate_dna_distances.main()
        ...
        mock_sys_exit.assert_called_once_with(1)
        self.assertIn(
            "Error: Need at least two sequences to calculate distances.",
            stderr_capture.getvalue(),
        )
    ...
    def test_empty_sequences_prints_na_distances(self, mock_sys_exit, mock_parse_fasta):
        ...
        self.assertIn(
            "Pair\tRaw_Diffs\tValid_Sites\tProp_Diff\tJC69_d\tJC69_SE", output
        )  # Check header
        self.assertIn(
            "seq1 vs seq2\t0\t0\tN/A\tN/A\tN/A", output
        )  # Check data line for N/A
    ...
    def test_sequences_with_invalid_chars_ignored(self, mock_parse_fasta):
        ...
        mock_parse_fasta.return_value = [
            ...
            MockSeqRecord(
                "----CCCC", id="seq4"
            ),  # NNNN vs ---- (0 valid), AAAA vs CCCC (4 diffs, 4 valid)
        ]
        with patch.object(sys, "argv", ["calculate_dna_distances.py", "dummy.fasta"]):
            with redirect_stdout(io.StringIO()) as stdout_capture:
                calculate_dna_distances.main()
    ...
    def test_calculate_tajima_nei_defined_symmetric_freqs(self):
        ...
        base_counts_all_seqs = {
            "A": 1,
            "C": 1,
            "G": 1,
            "T": 1,
        }  # Gives equal freqs (0.25)
        valid_sites = 2
        ...
        d, se = calculate_dna_distances.calculate_tajima_nei(
            ...
            valid_sites,
            P_prop=0.0,
            Q_prop=1.0,  # P_prop=transitions/valid, Q_prop=transversions/valid
        )
        self.assertAlmostEqual(
            d, 1.2978, places=4
        )  # Value from previous manual calculation
        self.assertEqual(se, "N/A")
    ...
    def test_calculate_tamura_nei_defined_example(self):
        ...
        self.assertEqual(
            d, "Inf"
        )  # Because Q=1 and equal base freqs makes log args zero
        self.assertEqual(se, "N/A")
    ...
    def test_main_loop_p_q_aggregation(self, mock_parse_fasta):
        ...
        self.assertIn(
            "s1 vs s2\t2\t4\t0.5000\t0.5000\t0.0000", output
        )  # Raw, Valid, Prop, P_ts, Q_tv
        self.assertIn(
            "0.8240\tN/A", output
        )  # JC69 d, SE (SE for P=0.5 is N/A by formula (1-4/3P)^2 in denom)
    ...
    def test_output_formatting_precision_and_na_inf(self, mock_parse_fasta):
        mock_parse_fasta.return_value = [
            ...
            MockSeqRecord(
                "TTTT", id="s3"
            ),  # s1 vs s3: P=0, Q=1 (all diffs are transversions)
        ]
        with patch.object(sys, "argv", ["calculate_dna_distances.py", "dummy.fasta"]):
            with redirect_stdout(io.StringIO()) as stdout_capture:
                calculate_dna_distances.main()
        ...
        self.assertIn(
            "s1 vs s2\t0\t4\t0.0000\t0.0000\t0.0000\t0.0000\t0.0000\t0.0000\t0.0000\tN/A",
            output,
        )
        ...
        self.assertIn(
            "s1 vs s3\t4\t4\t1.0000\t0.0000\t1.0000\tInf\tN/A\tInf\tN/A\t0.0000", output
        )
    ...
    def test_output_header_presence_and_content(self):
        ...
        with patch(
            ...
            sys, "argv", ["calculate_dna_distances.py", "dummy.fasta"]
        ), redirect_stdout(
            io.StringIO()
        ) as stdout_capture:
