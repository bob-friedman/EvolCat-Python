...
class TestFindBlastTopPairs(unittest.TestCase):
    ...
    def _run_main_with_args(self, arg_list, stdin_data=None, mock_file_data=None):
        ...
        open_mock = mock_open(
            read_data=mock_file_data if mock_file_data is not None else ""
        )
        ...
        with patch.object(sys, "argv", full_argv), patch("sys.stdin", mock_stdin), (
            patch("builtins.open", open_mock) if input_file_path else MagicMock()
        ), redirect_stdout(io.StringIO()) as captured_stdout, redirect_stderr(
            io.StringIO()
    ...
    def test_arg_parser_defaults(self):
        with patch(
            "pylib.scripts.find_blast_top_pairs.process_input_stream"
    ...
    def test_arg_parser_custom_values(self):
        ...
        with patch(
            ...
            self._run_main_with_args(
                custom_args_list, mock_file_data=""
            )  # Provide data for mocked open
            self.assertTrue(mock_process_stream.called)
            passed_args = mock_process_stream.call_args[0][1]
            self.assertEqual(passed_args.input_file, "test.blast")
            self.assertEqual(passed_args.query_id_col_idx, 1)
            self.assertEqual(passed_args.subject_id_col_idx, 2)
            self.assertEqual(passed_args.score_col_idx, 10)
            self.assertEqual(passed_args.filter_col_idx, 4)
    ...
    def test_input_from_file(self):
        ...
        stdout, _, _ = self._run_main_with_args(
            ["--input_file", "data.txt"], mock_file_data=file_data
        )
    ...
    def test_input_file_not_found_error(self):
        ...
        with patch(
            ...
            _, stderr, mock_exit = self._run_main_with_args(
    ...
    def test_output_fewer_than_four_fields_input(self):
        ...
        stdout_3_fields, stderr_3_fields, _ = self._run_main_with_args(
            args_custom_idx, stdin_data=data_3_fields_parseable
        )
        ...
        data_short_for_indices = (
            "q1\ts1\n"  # query_idx=0, subject_idx=1. score_idx=2 (default) is missing.
        )
        stdout_short, stderr_short, _ = self._run_main_with_args(
            [], stdin_data=data_short_for_indices
        )
        self.assertEqual(stdout_short, "")  # No output
        self.assertIn(
            "Skipping line (not enough columns or conversion error)", stderr_short
        )
    ...
    def test_warning_for_malformed_line_non_numeric(self):
        ...
        _, stderr, _ = self._run_main_with_args([], stdin_data=data)
        self.assertIn(
            "Skipping line (not enough columns or conversion error): ValueError",
            stderr.upper(),
        )  # Check for ValueError part
    ...
    def test_warning_for_malformed_line_too_few_cols_for_indices(self):
        ...
        _, stderr, _ = self._run_main_with_args([], stdin_data=data)
        self.assertIn(
            "Skipping line (not enough columns or conversion error): IndexError",
            stderr.upper(),
        )
        self.assertIn("Original line: q1\ts1\t100", stderr)
