...
class MockSeqRecord:
    def __init__(self, seq_str, id="test_id", name="", description=""):
        ...
        self.seq = MockBioSeq(
            seq_str
        )  # Use our MockBioSeq directly for clarity in record
...
class MockBioSeq:
    ...
    def reverse_complement(self):  # Method called by script
        ...
        rev_comp_str = "".join(
            complement.get(base, base) for base in reversed(self._data)
        )
...
class TestDotPlot(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.seq_patcher = patch(
            "Bio.Seq.Seq", side_effect=lambda data, *args, **kwargs: MockBioSeq(data)
        )
    ...
    @patch("sys.exit")
    def test_sequence_input_file_not_found(
        self, mock_exit, MockDotPlotter, mock_parse_fasta
    ):
        ...
        mock_parse_fasta.side_effect = [
            MockSeqRecord("ACGT", "s1"),
            FileNotFoundError("s2.fasta not found"),
        ]
        with patch.object(
            sys,
            "argv",
    ...
    @patch("sys.exit")
    def test_sequence_input_empty_fasta(
        self, mock_exit, MockDotPlotter, mock_parse_fasta
    ):
    ...
    def test_generate_dot_file_no_matches(self, mock_file_open):
        ...
        plotter = dot_plot.DotPlotter(
            args, "s1", "s2", MockBioSeq("AAA"), MockBioSeq("TTT")
        )
    ...
    def test_generate_dot_file_simple_direct_repeats(self, mock_file_open):
        ...
        plotter = dot_plot.DotPlotter(
            args, "s1", "s2", MockBioSeq("ABCDEFG"), MockBioSeq("XYZABCXYZ")
        )
        plotter.generate_dot_file()
        handle = mock_file_open()
    ...
    def test_generate_dot_file_simple_inverted_repeat(self, mock_file_open):
        ...
        plotter = dot_plot.DotPlotter(
            args, "s1", "s2", MockBioSeq("ABCXYZ"), MockBioSeq("MNOGCBDEF")
        )
        plotter.generate_dot_file()
        handle = mock_file_open()
        ...
        # Expected: 0 \t 0
        plotter_rc = dot_plot.DotPlotter(
            args, "s1rc", "s2rc", MockBioSeq("GAT"), MockBioSeq("ATC")
        )
    ...
    def test_generate_dot_file_wordlen_step(self, mock_file_open):
        ...
        plotter = dot_plot.DotPlotter(
            args, "s1", "s2", MockBioSeq("ABABABA"), MockBioSeq("CDABABEF")
        )
        plotter.generate_dot_file()
        handle = mock_file_open()
        expected_calls = [
    ...
    def test_generate_dot_file_skip_n_chars(self, mock_file_open):
        ...
        plotter = dot_plot.DotPlotter(
            args, "s1", "s2", MockBioSeq("ABCNNNXYZ"), MockBioSeq("DEFABCXYZ")
        )
        plotter.generate_dot_file()
        handle = mock_file_open()
        handle.write.assert_any_call("0\t3\n")
    ...
    @patch("builtins.open", new_callable=mock_open)
    def test_plot_generation_from_dotfile(
        self, mock_file_open_dot, mock_subplots, mock_scatter, mock_show, mock_savefig
    ):
        mock_ax = MagicMock()
        mock_fig = MagicMock()
        ...
        plotter = dot_plot.DotPlotter(
            ...
            seq1_name,
            seq2_name,
            MockBioSeq("A" * seq1_len),
            MockBioSeq("C" * seq2_len),
        )
        ...
        found_s2_label = any(
            c[0][2] == seq2_name and "vertical" in c[1].get("rotation", "")
            for c in mock_ax.text.call_args_list
        )
    ...
    @patch("matplotlib.pyplot.subplots")
    @patch("builtins.open", new_callable=mock_open)
    def test_plot_empty_or_invalid_dotfile(
        self, mock_file_open_dot, mock_subplots, mock_scatter
    ):
        mock_ax = MagicMock()
        ...
        plotter_empty = dot_plot.DotPlotter(
            args, "s1", "s2", MockBioSeq("A"), MockBioSeq("C")
        )
        ...
        with redirect_stderr(io.StringIO()) as err:
            plotter_invalid = dot_plot.DotPlotter(
                args, "s1", "s2", MockBioSeq("A" * 10), MockBioSeq("C" * 10)
            )
    ...
    def test_error_sequences_too_short_for_wordlen(self, mock_exit, mock_parse_fasta):
        ...
        with patch.object(
            ...
            with redirect_stderr(io.StringIO()) as err:
                dot_plot.main()
        self.assertIn(
            "Error: Sequence s1 (length 1) is shorter than word length (3).",
            err.getvalue(),
        )
        mock_exit.assert_called_with(1)
        ...
        with patch.object(
            ...
            with redirect_stderr(io.StringIO()) as err:
                dot_plot.main()
        self.assertIn(
            "Error: Sequence s2 (length 2) is shorter than word length (3).",
            err.getvalue(),
        )
        mock_exit.assert_called_with(1)
    ...
    def test_end_to_end_successful_run(
        self,
        mock_sys_exit,
        mock_show,
        mock_savefig,
        ...
        mock_dot_write_handle.write.assert_any_call(
            "0\t3\n"
