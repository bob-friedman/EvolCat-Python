...
def calculate_consensus(alignment, threshold, ambiguous_char, require_multiple, inferred_alphabet):
    ...
    consensus_sequence = []
    alignment_len = alignment.shape[1]
    ...
    for i in range(alignment_len):
        ...
        if best_freq >= threshold:
            # Check the require_multiple condition: if diversity is high (>= require_multiple),
            # force ambiguity even if the best character meets the threshold.
            if require_multiple > 1 and len(valid_chars_counts) >= require_multiple:
...
def calculate_stats(alignment, inferred_alphabet):
    ...
    stats = {}
    num_sequences = len(alignment.sequences)
...
def main():
    ...
    if args.get_consensus or args.get_stats: # Only print if relevant
        print(f"Inferred alphabet type: {inferred_alphabet}", file=sys.stderr)
