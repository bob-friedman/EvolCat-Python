...
# This ensures that 'blast_ncbi_seq_def' can be imported
...
class TestBlastNcbiSeqDef(unittest.TestCase):
    ...
    def _mock_requests_post_response(self, status_code=200, text_data="", json_data=None, raise_for_status_exception=None):
    ...
    def test_successful_blast_workflow(self, mock_sleep, mock_post):
        ...
        with redirect_stdout(captured_output):
            ...
            with patch.object(sys, 'argv', test_args):
                ...
                self.assertIsNone(cm.exception.code, "Script should exit cleanly on success.")
    ...
    def test_blast_rid_unknown(self, mock_sleep, mock_post):
        mock_post.return_value = self._mock_requests_post_response(text_data="RID = UNKNOWN")
        path = self._create_dummy_fasta(self.test_fasta_content_single)
        ...
        with redirect_stdout(captured_output):
            ...
            with patch.object(sys, 'argv', test_args):
                with self.assertRaises(SystemExit) as cm:
                    blast_ncbi_seq_def.main()
                self.assertIsNone(cm.exception.code, "Script should exit after 'bad input'.")
    ...
    def test_blast_timeout(self, mock_sleep, mock_post):
        mock_post.side_effect = [self._mock_requests_post_response(text_data="RID = TESTRID456")] + \
                                [self._mock_requests_post_response(text_data="Status=WAITING")] * blast_ncbi_seq_def.MAX_ATTEMPTS
        ...
        with redirect_stdout(captured_output):
            ...
            with patch.object(sys, 'argv', test_args):
                with self.assertRaises(SystemExit) as cm:
                    blast_ncbi_seq_def.main()
                self.assertIsNone(cm.exception.code, "Script should exit after timeout.")
    ...
    def test_blast_submit_http_error(self, mock_sleep, mock_post):
        http_error_exception = requests.exceptions.RequestException("Test HTTP Error on Submit")
        mock_post.return_value = self._mock_requests_post_response(status_code=500, raise_for_status_exception=http_error_exception)
        ...
        with redirect_stdout(captured_output):
            ...
            with patch.object(sys, 'argv', test_args):
                with self.assertRaises(SystemExit) as cm:
                    blast_ncbi_seq_def.main()
                self.assertIsNone(cm.exception.code, "Script should exit on submit error.")

        output = captured_output.getvalue()
        self.assertIn("Error submitting BLAST request: Test HTTP Error on Submit", output)
        self.assertEqual(mock_post.call_count, 1)
    ...
    def test_blast_poll_http_error(self, mock_sleep, mock_post):
        http_error_exception = requests.exceptions.RequestException("Test HTTP Error on Poll")
        mock_post.side_effect = [
            self._mock_requests_post_response(text_data="RID = TESTRID789"), 
            self._mock_requests_post_response(status_code=500, raise_for_status_exception=http_error_exception)
        ]
        ...
        with redirect_stdout(captured_output):
            ...
            with patch.object(sys, 'argv', test_args):
                with self.assertRaises(SystemExit) as cm:
                    blast_ncbi_seq_def.main()
                self.assertIsNone(cm.exception.code, "Script should exit on poll error.")
    ...
    def test_argument_usage_in_main(self, mock_poll, mock_submit, mock_initial_sleep):
        ...
        with patch.object(sys, 'argv', test_args):
            with self.assertRaises(SystemExit) as cm:
                blast_ncbi_seq_def.main()
            self.assertIsNone(cm.exception.code, "main() should exit cleanly with mocked functions.")
    ...
    def test_blast_rid_not_found_general(self, mock_sleep, mock_post):
        mock_post.return_value = self._mock_requests_post_response(text_data="Some other response without RID")
        path = self._create_dummy_fasta(self.test_fasta_content_single)
    ...
    def test_blast_status_not_found(self, mock_sleep, mock_post):
        mock_post.side_effect = [
            ...
            self._mock_requests_post_response(text_data="Some response without Status=") 
        ]
        path = self._create_dummy_fasta(self.test_fasta_content_single)
    ...
    def test_blast_status_not_found_but_ready_lenient(self, mock_sleep, mock_post):
        ...
        output = captured_output.getvalue()
        self.assertIn("Could not find Status in response:", output) 
        self.assertIn("Assuming READY based on content, though Status line missing.", output)
...
if __name__ == '__main__':
    unittest.main(argv=[sys.argv[0]] + sys.argv[1:],verbosity=2, exit=False)
