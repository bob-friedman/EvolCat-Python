...
class TestQueryNcbiEntrez(unittest.TestCase):
    ...
    def test_argument_parsing_fetch_db_sync(self, mock_get_ua):
        ...
        with patch.object(
            sys,
            "argv",
            ["query_ncbi_entrez.py", "--term", "test_term", "--search_db", "nuccore"],
        ):
        ...
        with patch.object(
            ...
            if not any(arg.startswith("--fetch_db") for arg in sys.argv):
                args.fetch_db = (
                    args.search_db
                )  # This line won't run if --fetch_db is present
            self.assertEqual(
                args.fetch_db, "protein"
            )  # Should remain protein as it was specified
    ...
    def test_make_request_success_first_try(self, mock_sleep, mock_get, mock_get_ua):
        ...
        response = query_ncbi_entrez._make_request_with_retries(
            "fake_url", {}, api_key_provided=False
        )
    ...
    def test_make_request_retry_then_success(self, mock_sleep, mock_get, mock_get_ua):
        ...
        response = query_ncbi_entrez._make_request_with_retries(
            "fake_url", {}, max_retries=2, api_key_provided=False
        )
        self.assertIsNotNone(response)
        self.assertEqual(response.text, "success")
        self.assertEqual(mock_get.call_count, 2)
        # Sleep is called after a failure, then for retry backoff.
    ...
    def test_make_request_all_retries_fail(self, mock_sleep, mock_get, mock_get_ua):
        ...
        with redirect_stderr(io.StringIO()) as stderr_capture:
            response = query_ncbi_entrez._make_request_with_retries(
                "fake_url", {}, max_retries=3, api_key_provided=True
            )
        self.assertIsNone(response)
        self.assertEqual(mock_get.call_count, 3)
        # Sleeps for rate limiting (if REQUEST_COUNT > 0) and then for retry backoff.
        # Exact calls depend on timing factors, ensure sleep is called.
    ...
    def test_make_request_rate_limiting_sleep(self, mock_sleep, mock_get, mock_get_ua):
        ...
        query_ncbi_entrez._make_request_with_retries(
            "fake_url1", {}, api_key_provided=False
        )
        ...
        query_ncbi_entrez._make_request_with_retries(
            "fake_url2", {}, api_key_provided=True
        )
    ...
    def test_search_entrez_success(self, mock_make_request, mock_get_ua):
        ...
        uids, webenv, qk = query_ncbi_entrez.search_entrez(
            "term", "db", "url", "email", "key", 3
        )
    ...
    def test_search_entrez_no_uids(self, mock_make_request, mock_get_ua):
        ...
        mock_make_request.return_value = self._mock_response(text_data=xml_response)
        with redirect_stderr(io.StringIO()) as stderr_capture:
            uids, _, _ = query_ncbi_entrez.search_entrez(
                "term", "db", "url", None, None, 1
            )
    ...
    def test_search_entrez_ncbi_error_xml(self, mock_make_request, mock_get_ua):
        ...
        self.assertIn(
            "NCBI E-utility Error: Invalid db name", stderr_capture.getvalue()
        )
    ...
    def test_fetch_entrez_records_with_webenv(self, mock_make_request, mock_get_ua):
        mock_make_request.return_value = self._mock_response(
            text_data="Sample GenBank Record"
        )
        ...
        # Check that WebEnv and query_key were included in the request parameters.
        self.assertIn("WebEnv", kwargs["params"])
        self.assertIn("query_key", kwargs["params"])

    @patch("query_ncbi_entrez._make_request_with_retries")
    def test_fetch_entrez_records_with_uid_list_get(
        self, mock_make_request, mock_get_ua
    ):
        mock_make_request.return_value = self._mock_response(text_data="FASTA >seq1...")
        with redirect_stdout(io.StringIO()) as stdout_capture:
            success = query_ncbi_entrez.fetch_entrez_records(
                "db", "fasta", "text", "url", None, None, 1, uid_list=["1", "2", "3"]
            )
        self.assertTrue(success)
    ...
    @patch("query_ncbi_entrez._make_request_with_retries")
    def test_fetch_entrez_records_with_uid_list_post(
        self, mock_make_request, mock_get_ua
    ):
        # Simulate a long list of UIDs that should trigger POST
        long_uid_list = [str(i) for i in range(250)]
        mock_make_request.return_value = self._mock_response(
            text_data="Long FASTA data..."
        )

        with redirect_stdout(io.StringIO()) as stdout_capture:
            success = query_ncbi_entrez.fetch_entrez_records(
                "db", "fasta", "text", "url", None, None, 1, uid_list=long_uid_list
            )
        self.assertTrue(success)
        args, kwargs = mock_make_request.call_args
    ...
    def test_main_successful_flow(self, mock_fetch, mock_search, mock_get_ua):
        ...
        mock_fetch.assert_called_once_with(
            ...
            "gb",
            "text",
            query_ncbi_entrez.BASE_EUTILS_URL,
            "a@b.com",
            None,
            3,
            uid_list=["123", "456"],
    ...
    @patch("query_ncbi_entrez.search_entrez")
    @patch(
        "query_ncbi_entrez.fetch_entrez_records"
    )  # Also mock fetch to prevent it running
    def test_main_search_no_results(self, mock_fetch, mock_search, mock_get_ua):
        ...
        with patch.object(sys, "argv", test_args):
            with redirect_stdout(io.StringIO()) as stdout_capture:
                ...
                self.assertEqual(
                    cm.exception.code, 0
                )  # Successful exit even if no results

        self.assertIn(
            "No records found for term 'term_no_results'", stdout_capture.getvalue()
        )
        mock_fetch.assert_not_called()  # Fetch should not be called if no UIDs from search.

    @patch("query_ncbi_entrez.search_entrez")
    def test_main_search_fails(self, mock_search, mock_get_ua):
        mock_search.return_value = (None, None, None)  # Simulate search error
    ...
    def test_main_fetch_fails(self, mock_fetch, mock_search, mock_get_ua):
        ...
        mock_fetch.return_value = False  # Simulate fetch error
